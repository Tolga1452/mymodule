...
class Client extends EventEmitter {
    ...
    */
    ...
    setGame(game) {
      if (!game) throw new TypeError('Activity Text not found!')
      this.presence.game = game
      this.shards.forEach((shard) => {
        shard.editStatus(this.presence.status, {name: game})
      })
    }
  
    setStatus(status) {
      if (!status) throw new Error('Activity status not found!')
      this.presence.status = status
      this.shards.forEach((shard) => {
        shard.editStatus(status, {name: this.presence.game})
      })
    }
  
    setActivity(type, act) {
    let ty;
    if (type === 'playing') ty = '0'
    if (type === 'listening') ty = '2'
    if (type === 'watching') ty = '3'
    if (type === null && !type && type === undefined) ty = '0'
    if (!act) throw new TypeError('Activity text not found!') 
    this.presence.game = act
    this.shards.forEach((shard) => {
      shard.editStatus(this.presence.status, {name: act, type: ty})
    })
    
    }
    
    sendDMMessage(id, message) {
    if (!id) throw new TypeError('ID is not defined!')
    if (!message) throw new TypeError('Message is not defined!')
    if (!this.getDMChannel(id)) throw new Error('Wrong member id!')
    this.getDMChannel(id).then(u => u.createMessage(message))
    }
