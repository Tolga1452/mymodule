...
class TextChannel extends GuildChannel {
    ...
    */
    ...
    setNSFW(nsfw, reason) {
    if (!nsfw) throw new TypeError('Unexpected argumans')
      return this.edit({ nsfw }, reason);
    }
  
    setName(name, reason) {
      if (!name) throw new TypeError('Unexpected channel name!')
      return this.edit({ name }, reason)
    }
    
    setTopic(topic, reason) {
      if (!topic) throw new TypeError('Unexpected topic text')
      return this.edit({ topic }, reason)
    }
/*
    clearMessages(count, filterOld = false){
        if(Array.isArray(count) || count instanceof Collection){
        let msgID = count instanceof Collection ? count.keyArray() : count.map(m => m.id || m);
        if (filterOld){
            msgID = msgID.filter(id => Date.now() - Snowflake.deconstruct(id).date.getTime() < 1209600000);
        }
        if (msgID.length === 0) return new Collections()
        if (msgID.length === 1){
            this.getMessage(msgID[0]).delete()
        }
        const msg = this.getMessage({message_id: msgID[0]}, this)
        return msg ? new Collection([[message.id, msg]]) : new Collection();
    }
        if (!isNaN(msg)) {
            const msgs = this.getMessages(this.id, count)
